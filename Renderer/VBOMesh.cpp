...
bool VBOMesh::loadOBJTest(const char * path)
{
	...
	indexVBO(in_vertices, in_uvs, in_normals, out_indices, out_vertices, out_uvs, out_normals);

	if (genTang && out_uvs.size() > 0) {
		generateTangents(out_vertices, out_normals, out_indices, out_uvs, out_tangents);
	}

	
	storeVBO(out_vertices, out_normals, out_uvs, out_tangents, out_indices);
	return true;
}

void VBOMesh::indexVBO(std::vector<glm::vec3>& in_vertices, std::vector<glm::vec2>& in_uvs, std::vector<glm::vec3>& in_normals, std::vector<int>& out_indices, std::vector<glm::vec3>& out_vertices, std::vector<glm::vec2>& out_uvs, std::vector<glm::vec3>& out_normals)
{
...
bool VBOMesh::getSimilarVertexIndex_fast(PackedVertex & packed, std::map<PackedVertex, int>& VertexToOutIndex, int & result)
{
...
void VBOMesh::storeVBO( const std::vector<glm::vec3> & points,
                        const std::vector<glm::vec3> & normals,
                        const std::vector<glm::vec2> &texCoords,
